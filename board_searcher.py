...
class MultiAgentSearchAgent(Agent):
    ...
    def __init__(self, evaluation_function='scoreEvaluationFunction', depth=DEPTH):
...
#
#     def rec_minmax(self, turn, depth, game_board):
#         '''
#         a recursive algorithm to calculate minimax algorithm
#         :param depth: tracking our depth in the tree
#         :param game_board: A stateof the game
#         :param player: 0 - player, 1 - computer
#         :return: value of the state according to minimax
#         '''
#         if depth == 0:
#             return self.evaluation_function(game_board)
#         # minimize
#         if round(depth) != depth:  # depth = something.5
#             opp = switch_turn(turn)
#             return return_min([self.rec_minmax(opp, depth - 0.5, game_board.generate_successor(opp, move)) \
#                                for move in game_board.get_actions_to_explore()])
#         # maximize
#         else:
#             return return_max([self.rec_minmax(turn, depth - 0.5, game_board.generate_successor(turn, move)) \
#                                for move in game_board.get_actions_to_explore()])
#
#     def get_action(self, game_board, turn):
#         """
...
#         game_board.get_actions_to_explore(agent_index):
#             Returns a list of legal actions for an agent
#             agent_index=0 means our agent, the opponent is agent_index=1
#         Action.STOP:
#             The stop direction, which is always legal
#         game_board.generate_successor(agent_index, action):
#             Returns the successor game state after an agent takes an action
#         """
#         moves = dict()
#         for move in game_board.get_actions_to_explore():
#             move_value = self.rec_minmax(turn, self.depth - 0.5, game_board.generate_successor(turn, move))
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def rec_alpha_beta(self, turn, depth, game_board, alpha, beta):
        ...
        if round(depth) != depth:  # depth = something.5
            opp = switch_turn(turn)
            min_value = np.inf
            moves_values = dict()  # TODO: for debug
            ...
            for successor in sorted_successors:
                ...
                if move_value == -np.inf:
                    return -np.inf
                moves_values[successor.last_action] = move_value  # TODO: for debug
    ...
    def get_action(self, game_board, turn):
        ...
        # print("white value: %d, black value: %d" %(game_board.black_value, game_board.white_value))
